...
@Autonomous
@Config
public class AutonomaRedDreapta extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        robot = new SampleMecanumDrive(hardwareMap, Orientation.RIGHT);
        dashboard = FtcDashboard.getInstance();
        while(opModeInInit()) caz = robot.recognition.getCaz(2);
        waitForStart();

        // caz stanga
        TrajectorySequence traj_stanga = robot.trajectorySequenceBuilder(new Pose2d(-35.3, 67, Math.toRadians(270))) // start!!
                .forward(15)
                .lineToLinearHeading(new Pose2d(x_stanga, y_stanga, Math.toRadians(20)))
                .back(15)
                // .strafeRight(40)
                // .forward(50)
...
//                .lineToLinearHeading(new Pose2d(-21, 30, Math.toRadians(20)))
                .back(15)
                // .strafeRight(40)
                // .forward(50)
                // .lineTo(new Vector2d(47, 34.8))
                .build();
//        robot.followTrajectorySequence(traj_center);
